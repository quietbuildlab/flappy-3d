...
- Phone and tablet focus checks use native touchscreen input with Playwright-controlled time to exercise deferred focus handling without CI command latency determining collisions. They verify repeated taps, Pause/Resume, and external focus in an independent round. The cream layout test also holds time only while entering Pause, then resumes real-time gameplay through the natural collision and results. Other gameplay suites and normal-browser release smoke checks use real wall time.

...
- Phone and tablet focus checks use native touchscreen input with Playwright-controlled time to exercise deferred focus handling without CI command latency determining collisions. They verify repeated taps, Pause/Resume, and external focus in an independent round. Other gameplay suites and normal-browser release smoke checks use real wall time.

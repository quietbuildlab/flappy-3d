...
- `vite.config.ts`, `tsconfig.json`, `playwright.config.ts`, `package.json`, and `package-lock.json` define build, test, and dependency behavior.
- `scripts/` contains verification and asset scripts. Keep scripts runnable from npm commands when they gate releases or CI.
...
- `src/ui/styles.css` styles the overlay UI.
